...
function addTokensToAst(tokens, ast) {
    for (let i = 0; i < tokens.length; i++) {
        let indexOffset = addAstNode(ast, tokens, i);
        i += indexOffset;
    }
}
export function markdownTokenParser(tokens) {
    let ast = new Array(0);
    addTokensToAst(tokens, ast);
    return ast;
}
function getNewAstNode() {
    ...
    astNode.childNodes = new Array(0);
    return astNode;
}
function addAstNode(ast, tokens, tokenIndex) {
    let astNode = getNewAstNode();
    ...
    if (token.type == TokenType.HEADER) {
        ...
        let headerLevel = 1;
        while (tokenIndex + indexOffset + 1 < tokens.length - 1 &&
        ...
        if (tokens[tokenIndex + indexOffset + 1].type === TokenType.WHITESPACE) {
            ...
            let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + indexOffset + 1, TokenType.NEWLINE);
            let content = getTokensAsString(contentTokens);
            let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.ITALICS) {
        if (checkIfTypeIsFoundBeforeOtherType(tokens, tokenIndex + 1, TokenType.ITALICS, TokenType.NEWLINE)) {
            let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.ITALICS);
            let content = getTokensAsString(contentTokens);
            let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.BOLD) {
        if (checkIfTypeIsFoundBeforeOtherType(tokens, tokenIndex + 1, TokenType.BOLD, TokenType.NEWLINE)) {
            let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.BOLD);
            let content = getTokensAsString(contentTokens);
            let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.STRIKETHROUGH) {
        if (checkIfTypeIsFoundBeforeOtherType(tokens, tokenIndex + 1, TokenType.STRIKETHROUGH, TokenType.NEWLINE)) {
            let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.STRIKETHROUGH);
            let content = getTokensAsString(contentTokens);
            let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.UNORDERED_LIST_ITEM ||
        ...
        while (listItemTokenIndex + tokensSkippedForWhitespace < tokens.length &&
            ...
            let listItemAstNode = getNewAstNode();
            listItemAstNode.type = AstNodeType.LIST_ITEM;
            // Get its content tokens
            let contentTokens = getAllTokensUntilTokenReached(tokens, listItemTokenIndex + 1, TokenType.NEWLINE);
            let content = getTokensAsString(contentTokens);
            let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.IMAGE_START) {
        let altTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.BRACKET_END);
        let altText = getTokensAsString(altTokens);
        let altTextOffsetTokenLength = altTokens.length;
        ...
        if (tokens[tokenIndex + altTextOffsetTokenLength + 2].type ==
            TokenType.PAREN_START) {
            let imageTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + altTextOffsetTokenLength + 3, TokenType.PAREN_END);
            let imageUrl = getTokensAsString(imageTokens);
            let imageUrlOffsetTokenLength = imageTokens.length;
    ...
    if (token.type == TokenType.BRACKET_START) {
        let linkTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.BRACKET_END);
        let linkContent = getTokensAsString(linkTokens);
        let linkContentOffsetTokenLength = linkTokens.length;
        ...
        // We need to check if this is immediately followed by a parentheses
        if (tokens[tokenIndex + linkContentOffsetTokenLength + 2].type ==
            TokenType.PAREN_START) {
            let urlTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + linkContentOffsetTokenLength + 3, TokenType.PAREN_END);
            let urlContent = getTokensAsString(urlTokens);
            let urlContentOffsetTokenLength = urlTokens.length;
            // Let's create the Ast Node for the image
    ...
    if (token.type == TokenType.BLOCK_QUOTE) {
        let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.NEWLINE);
        let content = getTokensAsString(contentTokens);
        let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.CODE_BLOCK) {
        let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.CODE_BLOCK);
        let content = getTokensAsString(contentTokens);
        let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.INLINE_CODE) {
        if (checkIfTypeIsFoundBeforeOtherType(tokens, tokenIndex + 1, TokenType.INLINE_CODE, TokenType.NEWLINE)) {
            let contentTokens = getAllTokensUntilTokenReached(tokens, tokenIndex + 1, TokenType.INLINE_CODE);
            let content = getTokensAsString(contentTokens);
            let offsetTokenLength = contentTokens.length;
    ...
    if (token.type == TokenType.HORIZONTAL_LINE &&
...
function getOffsetOfTokensUntilTokenReached(tokens, startTokenIndex, stopTokenType) {
    let contentTokens = getAllTokensUntilTokenReached(tokens, startTokenIndex, stopTokenType);
    let content = "";
...
function checkIfTypeIsFoundBeforeOtherType(tokens, startTokenIndex, checkTokenType, otherTokenType) {
    let checkTokens = getAllTokensUntilTokenReached(tokens, startTokenIndex, checkTokenType);
    let otherTokens = getAllTokensUntilTokenReached(tokens, startTokenIndex, otherTokenType);
...
function getAllTokensUntilTokenReached(tokens, startTokenIndex, stopTokenType) {
    let responseTokens = new Array();
