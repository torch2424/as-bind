...
let tokens = new Array(0);
function isWhitespace(character) {
    return character.includes(" ");
}
function checkForTriplet(character, index, markdown) {
    return (markdown.charAt(index + 0).includes(character) &&
        markdown.charAt(index + 1).includes(character) &&
        markdown.charAt(index + 2).includes(character));
}
function addToken(markdown, tokenIndex, tokenValue) {
    let token = new Token();
    ...
    if (isWhitespace(tokenValue)) {
        ...
        tokenValue = "";
        while (tokenContinueLength < markdown.length - tokenIndex &&
...
export function markdownTokenizer(markdown) {
    tokens = new Array(0);
    for (let i = 0; i < markdown.length; i++) {
        let tokenValue = markdown.charAt(i);
