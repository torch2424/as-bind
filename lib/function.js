...
export function asbindApply(
  ...
  console.log("[ASBIND] Calling Export:");
  console.log(wasmExports);
  const startTime = performance.now();
  const responseRef = wasmExportFunction.apply(null, argumentRefs);
  const endTime = performance.now();
  console.log(`[ASBIND] Export call time: ${endTime - startTime}`);
  ...
  if (responseRef !== undefined) {
    ...
    if (!supportedType) {
      throw new Error(
        `The reference, ${responseRef}, is not a supported type by asbind`
      );
    }
